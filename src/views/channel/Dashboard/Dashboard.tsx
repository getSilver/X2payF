...
injectReducer('chDashboard', reducer)

const ChDashboard = () => {
    const dispatch = useAppDispatch()

    const { statisticData, leadByRegionData, recentLeadsData, emailSentData } =
        useAppSelector((state) => state.chDashboard.data.dashboardData)
    const loading = useAppSelector((state) => state.chDashboard.data.loading)
